...
public class Network {
    ...
    // MARK: -- ori
    @available(*, deprecated, message: "use request(_:transform:callbackQueue:progress:success:failure:)")
    @discardableResult
...
//    }
    ...
    static func transform(result: Result<Moya.Response, MoyaError>) -> Result<Any, NetworkError> {
        ...
        let transformResult: Result<Any, NetworkError>
                ...
                if let status = resObject?["status"] as? Int,
                   status != 200 {
                    ...
                    if let errCodeType = ServiceError(rawValue: status) {
                        transformResult = .failure(.server(errCodeType, errMsg))
                    } else {
                        transformResult = .failure(.undefined(status, errMsg))
                    }
                    ...
                    if let data = resObject?["data"] {
                        ...
                        transformResult = .failure(.undefined(-1, "map error"))
                    }
                }
            } catch let kError {
                if let kError = kError as? MoyaError {
                    transformResult = .failure(.moya(kError))
                } else {
                    transformResult = .failure(.system(kError))
                }
            }
        case .failure(let error):
            transformResult = .failure(.moya(error))
        }
        handleError(transformResult)
