...
class PFTextFieldRenderView: UIView {
    ...
    func startCursorViewAnimation(at index: Int) {
        if showCursor {
            ...
            opacityAni.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName.easeIn)
        ...
        if text != nil {
            ...
            if secureTextEntry {
                ...
                for (index, c) in text!.enumerated() {
                    ...
                    single.draw(at: textCenter, withAttributes: [NSAttributedString.Key.font : font, NSAttributedString.Key.foregroundColor : textColor!])
                }
            }
        }
    }
