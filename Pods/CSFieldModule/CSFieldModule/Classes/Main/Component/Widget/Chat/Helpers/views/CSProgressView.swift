...
@objc(KCSProgressView)
open class CSProgressView: UIView {
    public var store: CSChatCellStore
    ...
    //动画时间函数
    @objc
    public var timingFunction: CAMediaTimingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName.default)
    
    //进度更新动画过程中的回调，在这里可以拿到当前进度及进度条的frame
    @objc
    public var progressUpdating: ((CSChatCellStore ,Float, CGRect) -> ())?
    ...
    open override func layoutSubviews() {
        ...
        bounds.size.width *= CGFloat(1 - progress)
    ...
    @objc private func displayLinkAction() {
        ...
        let progress = (gradientLayer.frame.size.width - frame.size.width) / gradientLayer.frame.size.width
        progressUpdating?(self.store , Float(progress), frame)
    ...
    public func setProgress(_ progress: Float, animated: Bool) {
        ...
        CATransaction.setCompletionBlock {
            ...
            if duration == 0 {
                //更新回调
//                self.progressUpdating?(self.store, validProgress, self.maskLayer.frame)
            } else {
...
//                    self.progressUpdating?(self.store,validProgress, self.maskLayer.frame)
        ...
        bounds.size.width *= CGFloat(1 - validProgress)
