...
extension FriendService: TargetType, NetworkFullUrl {
    ...
    var path: String {
        ...
        case .breakConnection:
            return "/api/v0/break_connection"
        }
    }
    ...
    var task: Moya.Task {
        switch self {
        case .friends, .inviteFriends, .hiddenUsers, .action, .connectRequest, .inviteFieldFriends:
            return .requestPlain
        ...
        case .inviteJoinField(uid: let uid):
            ...
            return .uploadMultipart([id])
        case .breakConnection(uid: let uid):
            let parameters: [String: Any] = ["target_id": uid]
